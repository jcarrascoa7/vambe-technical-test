...
### Backend
- **`backend/api/insights.py`** (new) — `GET /insights/{chart_type}` endpoint that calls the LLM with domain-context prompts and returns Spanish explanatory text. Handles JSON and plain text responses, with fallback message on failure.
...
2. **Prompt returns JSON with `text` field** — Since `call_llm` doesn't force `responseMimeType: application/json`, the prompt asks the LLM to respond as `{"text": "..."}`. If the LLM returns plain text instead, it's used directly.
...
- All 134 tests pass (125 existing + 9 new insight tests).
