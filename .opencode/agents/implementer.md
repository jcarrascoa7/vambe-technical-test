...
7. **Write a summary** to `progress/impl_<feature_name>.md` with: files modified, key decisions, and any notes for the reviewer.
8. **Report completion** to the leader. Do NOT mark `done` yet. The leader will launch the reviewer.
9. **After reviewer approval**, the leader may launch you again for **session close**. When that happens:
   - Mark the feature as `done` in `feature_list.json`.
   - Move the summary from `progress/current.md` to the end of `progress/history.md`.
   - Empty `progress/current.md` leaving only the template.
   - Run `./init.sh` one final time to confirm everything is green.
10. **If blocked**, the leader may launch you to **close the blocked session**. When that happens:
